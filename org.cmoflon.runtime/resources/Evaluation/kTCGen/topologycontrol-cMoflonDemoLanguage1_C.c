#include "topologycontrol-cMoflonDemoLanguage1_C.h"
...
bool isWeightDefined(EDouble weight){
	return weight!=COMPONENT_NEIGHBORDISCOVERY_WEIGHTUNKNOWN;
}
...
// --- End of kTC-specific methods// --- End of user-defined helpers
...
#if COMPONENT_TOPOLOGYCONTROL == COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE1_C
PROCESS(component_topologycontrol, "topologycontrol: cMoflonDemoLanguage1_C");
PROCESS_THREAD(component_topologycontrol, ev, data) {
	...
	etimer_set(&waittime, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE1_C_UPDATEINTERVAL);
	while(1) {
		...
		tc.k = COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE1_C_K;
		ktcAlgorithm_run(&tc);
		LINK_T* onehop;
