#include "topologycontrol-cMoflonDemoLanguage_C.h"


//Start of declarations for hopcount usage
...
static void _hopcount_recv(const networkaddr_t *source, buffer_t *data, int8_t rssi) {
	...
	if(_hopcount_update()) {
		...
		etimer_set(&etimer_broadcast, random(CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_BROADCASTHOPCOUNT_IMMEDIATE_MIN, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_BROADCASTHOPCOUNT_IMMEDIATE_MAX));
	}
}
...
static void _hopcount_broadcast(s) {
	...
	if(etimer_broadcast.timer.interval <= CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_BROADCASTHOPCOUNT_IMMEDIATE_MAX)
		etimer_set(&etimer_broadcast, random(CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_BROADCASTHOPCOUNT_SMALLDELAY_MIN, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_BROADCASTHOPCOUNT_SMALLDELAY_MAX));
	else
		etimer_set(&etimer_broadcast, random(CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_BROADCASTHOPCOUNT_PERIODIC_MIN, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_BROADCASTHOPCOUNT_PERIODIC_MAX));
}
								...
								if (!node_equals(n2, this_node)) {
									if (!node_equals(n2, n3)) {
										...
										if(linkstate_equals(e12_marked, UNCLASSIFIED)){
											...
											if(lstarktcalgorithm_evaluateHopcountConstraint(_this,this_node_hopcount ,n2_hopcount ,n3_hopcount ,this_stretchFactor )){
											 LINK_T* e32;
...
#if COMPONENT_TOPOLOGYCONTROL == COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C
PROCESS(component_topologycontrol, "topologycontrol: cMoflonDemoLanguage_C");
PROCESS_THREAD(component_topologycontrol, ev, data) {
	...
	etimer_set(&waittime, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_UPDATEINTERVAL);
	while(1) {
		...
		tc.k = COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_K;
		tc.stretchFactor = COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE_C_STRETCHFACTOR;
