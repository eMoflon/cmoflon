...
/**
 * Copy of EMoflons ControlFlowModelAdaptor. Could not be imported because the package is not exported.
 *
 */
public class ControlFlowModelAdaptor extends ObjectModelAdaptor
{
   ...
   public Object getProperty(Interpreter interpreter, ST template, Object object, Object property, String propertyName) throws STNoSuchPropertyException
   {
      ...
      if (object instanceof PatternInvocation)
      {
         ...
         {
            ArrayList<VariableReference> boundParameters = new ArrayList<VariableReference>(invocation.getParameters().size());
         ...
         {
            ArrayList<VariableReference> freeParameters = new ArrayList<VariableReference>(invocation.getParameters().size());
            for (VariableReference reference : invocation.getParameters())
            {
               if (reference.isFree())
               {
         ...
         {
            return invocation.getCfNode().getId() + "_" + invocation.getPattern().eResource().getURI().fileExtension();
         }
      }
