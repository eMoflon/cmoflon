#include "topologycontrol-cMoflonDemoLanguage2_C-LStarKtcAlgorithm.h"
...
static void _hopcount_recv(const networkaddr_t *source, buffer_t *data, int8_t rssi) {
	...
	if(_hopcount_update()) {
		...
		etimer_set(&etimer_broadcast, random(CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_BROADCASTHOPCOUNT_IMMEDIATE_MIN, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_BROADCASTHOPCOUNT_IMMEDIATE_MAX));
	}
}
...
static void _hopcount_broadcast(s) {
	...
	if(etimer_broadcast.timer.interval <= CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_BROADCASTHOPCOUNT_IMMEDIATE_MAX)
		etimer_set(&etimer_broadcast, random(CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_BROADCASTHOPCOUNT_SMALLDELAY_MIN, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_BROADCASTHOPCOUNT_SMALLDELAY_MAX));
	else
		etimer_set(&etimer_broadcast, random(CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_BROADCASTHOPCOUNT_PERIODIC_MIN, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_BROADCASTHOPCOUNT_PERIODIC_MAX));
}
...
// --- End of user-defined helpers for LStarKtcAlgorithm
...
#if COMPONENT_TOPOLOGYCONTROL == COMPONENT_TOPOLOGYCONTROL_CMOFLONDEMOLANGUAGE2_C_LSTARKTCALGORITHM
PROCESS(component_topologycontrol, "topologycontrol: LStarKtcAlgorithm");
PROCESS_THREAD(component_topologycontrol, ev, data) {
	...
	etimer_set(&waittime, CLOCK_SECOND * COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_UPDATEINTERVAL);
	while(1) {
		...
		tc.k = COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_K;
		tc.stretchFactor = COMPONENT_TOPOLOGYCONTROL_LSTARKTCALGORITHM_STRETCHFACTOR;
